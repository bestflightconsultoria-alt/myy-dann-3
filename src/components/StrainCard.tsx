...
export const StrainCard: React.FC<StrainCardProps> = ({ strain, onClick }) => {
  // Define apenas uma etiqueta principal para o topo
  const displayBadge = () => {
  ...
  // Texto dinâmico de associações disponíveis
  ...
  return (
    ...
    >
      <div>
        {/* Topo com somente 1 etiqueta limpa */}
        ...
        {/* Aroma & Sabor */}
        {strain.aromaFlavor && (
          <p className="text-xs text-amber-900/80 bg-amber-50/70 border border-amber-200/70 px-2.5 py-1.5 rounded-lg mt-3 line-clamp-1 italic font-medium">
            🌿 {strain.aromaFlavor}
          </p>
        )}
      ...
      {/* Rodapé do Card com Botão Clicável */}
      <div className="mt-5 pt-3 border-t border-gray-100 flex items-center justify-between">
        ...
        <button
          ...
          className="px-3.5 py-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-600 hover:text-white rounded-xl border border-emerald-200 transition-all shadow-sm flex items-center gap-1"
        >
          Ver Preços →
        </button>
      </div>
